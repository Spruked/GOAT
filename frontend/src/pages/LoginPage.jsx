    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isLogin) {
          ...
          const hasCompletedOnboarding = localStorage.getItem('onboarding');
          const userData = { email: formData.email, isAdmin: false };
          
          if (!hasCompletedOnboarding && !userData.isAdmin) {
            // New users who haven't onboarded go to onboarding
          ...
          // For signup, automatically log in and go to onboarding
          localStorage.setItem('token', data.access_token);
          localStorage.setItem('user', JSON.stringify({ 
            email: formData.email,
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="guardian@eternal.vault"
                  className="w-full pl-12 pr-6 py-5 bg-cyan-950/40 border border-cyan-700/60 rounded-2xl text-xl text-cyan-100 placeholder-cyan-600 focus:outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-400/20 transition"
                />
              </div>
            </div>
