...
async def push_message(req: Request):
    ...
    try:
        ...
        return {
            ...
            "queue_size": len(MESSAGE_QUEUES[user_id]),
            "timestamp": time.time()
        }
...
@router.get("/pull")
async def pull_messages(user_id: str):
    """
    Pull messages from a user's queue
    """
    async with QUEUE_LOCK:
        if user_id not in MESSAGE_QUEUES:
            return {"messages": [], "count": 0}

        messages = MESSAGE_QUEUES[user_id].copy()
        # Clear the queue after pulling
        MESSAGE_QUEUES[user_id] = []
...
@router.get("/status")
async def get_queue_status(user_id: str = None):
    """
